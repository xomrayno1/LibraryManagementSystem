								...
								<div class="x_content">
									...
									<form:form servletRelativeAction="${searchUrl}" method="POST" modelAttribute="searchForm" cssClass="form-horizontal form-label-left">
										<div class="item form-group">
											<label class="col-form-label col-md-3 col-sm-3 label-align" for="fullName">Tên <span class="required">*</span>
											</label>
	...
	<div class="table-responsive">
		<a href='<c:url value="/user/add"></c:url>'><button class="btn btn-primary"><i class="glyphicon glyphicon-plus"></i>Thêm</button></a>
                      <table class="table table-striped jambo_table bulk_action">
                        ...
                        <tbody>
                          <c:forEach items="${listProduct}" var="user" varStatus="i"> 
                          	<tr>
                        	...
                        	<td>${user.cmnd}</td>                       	
                        	<td>${user.sdt}</td> 
                        	<c:forEach items="${user.userRolesDTOs}" var="item">
                        		 <td>${item.roleDTO.name}</td>  
                        	</c:forEach>                   	
                            <td colspan="3" class="last text-center">
                            	<input type="hidden" id="idProduct" value="${user.id}">
	                            <a href='<c:url value="/user/view/${user.id}"></c:url>' class="btn btn-primary"><i class="glyphicon glyphicon-eye-open"></i></a> 
	                            <a href='<c:url value="/user/edit/${user.id}"></c:url>' class="btn btn-warning"><i class="glyphicon glyphicon-edit"></i></a> 
		...
		<div id="deleteModal" class="modal fade">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<c:url value="/user/delete" var="urlDelete" />
