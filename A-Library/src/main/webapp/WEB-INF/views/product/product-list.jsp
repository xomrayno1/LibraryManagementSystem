...
<div class="right_col" role="main">
					...
					<div class="row">
						<div class="col-md-12 col-sm-12 ">
							<div class="x_panel">
								<div class="x_content">
									<br />
									<c:url value="/product-info/list/1" var="searchUrl"></c:url>
	...
	<div class="table-responsive">
		<a href='<c:url value="/product-info/add"></c:url>'><button class="btn btn-primary"><i class="glyphicon glyphicon-plus"></i>Thêm</button></a>
                      <table class="table table-striped jambo_table bulk_action">
                        ...
                        <tbody>
                          <c:forEach items="${listProduct}" var="product" varStatus="i"> 
                            ...
                            <td>${product.id}</td>
                            <td><img alt="" src='<c:url value="/resources/${product.imgUrl}"></c:url>' width="100px;" height="100px;"></td>
                        	<td>${product.name}</td>
                        	<td>${product.code}</td>
                        	<td>${product.authorDTO.name}</td>                        	
                            <td colspan="3" class="last text-center">
                            	<input type="hidden" id="idProduct" value="${product.id}">
	                            <a href='<c:url value="/product-info/view/${product.id}"></c:url>' class="btn btn-primary"><i class="glyphicon glyphicon-eye-open"></i></a> 
	                            <a href='<c:url value="/product-info/edit/${product.id}"></c:url>' class="btn btn-warning"><i class="glyphicon glyphicon-edit"></i></a> 
		...
		<div id="deleteModal" class="modal fade">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<c:url value="/product-info/delete" var="urlDelete" />
