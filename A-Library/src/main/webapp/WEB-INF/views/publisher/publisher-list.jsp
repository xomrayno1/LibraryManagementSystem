...
<div class="right_col" role="main">
					...
					<div class="row">
						<div class="col-md-12 col-sm-12 ">
							<div class="x_panel">
								<div class="x_content">
									<br />
									<c:url value="/publisher/list/1" var="searchUrl"></c:url>
	...
	<div class="table-responsive">
		<a href='<c:url value="/publisher/add"></c:url>'><button class="btn btn-primary"><i class="glyphicon glyphicon-plus"></i>Thêm</button></a>
                      <table class="table table-striped jambo_table bulk_action">
                        ...
                        <tbody>
                          <c:forEach items="${listProduct}" var="pubisher" varStatus="i"> 
                          	<tr>
                            <td>${pageInfo.offSet + i.index + 1} </td>
                            <td>${pubisher.id}</td>
                        	<td>${pubisher.name}</td>
                        	<td>${pubisher.code}</td>                       	
                        	<td>${pubisher.email}</td>
                        	<td>${pubisher.phone}</td>
                        	
                            <td colspan="3" class="last text-center">
                            	<input type="hidden" id="idProduct" value="${pubisher.id}">
	                            <a href='<c:url value="/publisher/view/${pubisher.id}"></c:url>' class="btn btn-primary"><i class="glyphicon glyphicon-eye-open"></i></a> 
	                            <a href='<c:url value="/publisher/edit/${pubisher.id}"></c:url>' class="btn btn-warning"><i class="glyphicon glyphicon-edit"></i></a> 
		...
		<div id="deleteModal" class="modal fade">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<c:url value="/publisher/delete" var="urlDelete" />
...
<script type="text/javascript">
	function gotoPage(page){
		$("#searchForm").attr('action',"<c:url value='/publisher/list/'/>"+page);
		$("#searchForm").submit();
	}
