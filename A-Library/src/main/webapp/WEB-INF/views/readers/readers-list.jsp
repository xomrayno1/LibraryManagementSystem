...
<div class="right_col" role="main">
					...
					<div class="row">
						<div class="col-md-12 col-sm-12 ">
							<div class="x_panel">
								<div class="x_content">
									<br />
									<c:url value="/readers/list/1" var="searchUrl"></c:url>
									<form:form servletRelativeAction="${searchUrl}" method="POST" modelAttribute="searchForm" cssClass="form-horizontal form-label-left">
										...
										<div class="item form-group">
											<label class="col-form-label col-md-3 col-sm-3 label-align" for="nameClass">Lớp <span class="required">*</span>
											</label>
											<div class="col-md-6 col-sm-6 ">
												<form:input path="nameClass" cssClass="form-control"/>
											</div>
										</div>
	...
	<div class="table-responsive">
		<a href='<c:url value="/readers/add"></c:url>'><button class="btn btn-primary"><i class="glyphicon glyphicon-plus"></i>Thêm</button></a>
                      <table class="table table-striped jambo_table bulk_action">
                        <thead>
                          <tr class="headings">
                            ...
                            <th class="column-title">Tạo thẻ thư viện</th>
                        ...
                        <tbody>
                          <c:forEach items="${listProduct}" var="readers" varStatus="i"> 
                          	<tr>
                            <td>${pageInfo.offSet + i.index + 1} </td>
                            <td>${readers.id}</td>                           
                        	<td>${readers.name}</td> 
                        	<td>${readers.mssv}</td> 
                        	<td>${readers.nameClass}</td>    
                        	<td > <a href='<c:url value="/library-card/add/${readers.id}"></c:url>' class="btn btn-info"><i class="glyphicon glyphicon-list-alt"></i></a></td>                                                         	
                            <td colspan="4" class="last text-center">
                            	<input type="hidden" id="idProduct" value="${readers.id}">
	                            <a href='<c:url value="/readers/view/${readers.id}"></c:url>' class="btn btn-primary"><i class="glyphicon glyphicon-eye-open"></i></a> 
	                            <a href='<c:url value="/readers/edit/${readers.id}"></c:url>' class="btn btn-warning"><i class="glyphicon glyphicon-edit"></i></a>
...
<script type="text/javascript">
	function gotoPage(page){
		$("#searchForm").attr('action',"<c:url value='/readers/list/'/>"+page);
		$("#searchForm").submit();
	}
	...
	$(document).ready(function(){
		$('#readerslistId').parents("li").addClass('active').siblings().removeClass("active");
		$('#readerslistId').addClass('current-page').siblings().removeClass("current-page");
		$('#readerslistId').parents().show();
	});
